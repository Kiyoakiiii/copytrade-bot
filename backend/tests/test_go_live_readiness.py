...
def test_manual_risk_service_forces_isolated_markets_to_three_x() -> None:
    class FakeClient:
        def __init__(self):
            self.updates = []
    ...
    assert client.updates == [("BTC", 3, False)]


def test_manual_risk_service_forces_cxmt_to_three_x_even_in_cross_mode() -> None:
    class FakeClient:
        def __init__(self):
            self.updates = []
    ...
    assert result.is_ok is True
    assert result.effective_leverage == 3
    assert client.updates == [("xyz:CXMT", 3, True)]
